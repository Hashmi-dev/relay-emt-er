...
function Trend({values}:{values:(number|null)[]}) {
  ...
  const points=values.map((v,i)=>v===null?null:[5+i*90/Math.max(1,values.length-1),27-(v-low)*21]);
...
export function Observations({state,persona,canEdit,act}:{state:EncounterState;persona:string;canEdit:boolean;act:(c:Command)=>Promise<Snapshot>}) {
  ...
  <section className="vitals-grid">{metrics.map(m=><article className="vital" key={m.key}><Definition label={m.label}/><strong>{m.key==="sbp"?String(last.values.sbp??"—")+"/"+String(last.values.dbp??"—"):last.values[m.key]??"—"}<small>{m.unit}</small></strong><Trend values={state.observations.slice(-8).map(o=>o.values[m.key])}/></article>)}</section>
  <Dialog open={open} onOpenChange={v=>!saving&&setOpen(v)}><DialogContent><DialogHeader><DialogTitle>Record simulated observations</DialogTitle><DialogDescription>Blank values are recorded as unknown. These readings are not from the Arduino.</DialogDescription></DialogHeader><form onSubmit={e=>{e.preventDefault();void save();}}><div className="input-grid">{Object.keys(last.values).map(key=><label key={key} className="field-label">{key==="sbp"?"Systolic BP":key==="dbp"?"Diastolic BP":metrics.find(m=>m.key===key)?.label}<Input type="number" step={key==="temp"?"0.1":"1"} value={values[key]||""} onChange={e=>setValues(v=>({...v,[key]:e.target.value}))}/></label>)}</div>{error?<p className="inline-error" role="alert">{error}</p>:null}<Button disabled={saving} type="submit">{saving?"Saving…":"Save observations"}</Button></form></DialogContent></Dialog></>;
}
