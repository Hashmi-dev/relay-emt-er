...
import { clearTelemetry, mutateSession, readSession, saveTelemetry, snapshot } from "@/lib/relay/store";
...
export async function POST(request: Request, context: { params: Promise<{ id: string; action: string }> }) {
  try {
    ...
    if (command.action === "evaluate") await evaluate(id, command.actor, command.mode, command.force);
